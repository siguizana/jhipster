...
export class TypeEtablissementComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        this.typeEtablissementService
            ...
            .subscribe(
                (res: HttpResponse<ITypeEtablissement[]>) => this.paginateTypeEtablissements(res.body, res.headers),
    ...
    trackId(index: number, item: ITypeEtablissement) {
        return item.id;
    }

    registerChangeInTypeEtablissements() {
        this.eventSubscriber = this.eventManager.subscribe('typeEtablissementListModification', response => this.reset());
    }
    ...
    protected paginateTypeEtablissements(data: ITypeEtablissement[], headers: HttpHeaders) {
        this.links = this.parseLinks.parse(headers.get('link'));
        this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
        for (let i = 0; i < data.length; i++) {
            this.typeEtablissements.push(data[i]);
        }
    }
